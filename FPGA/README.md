...
`UMH_7.ldf` targets `LCMXO2-2000HC-4MG132C`. The source clock is the 8 MHz STM32 PA8 HSE MCO; the checked-in `EHXPLLJ` generates the 128 MHz FPGA clock. Keep `UMH_7.lpf` as the source constraint file. Files generated under `UMH_7_1/` are build outputs and may be deleted and regenerated.

The design implements the STM32 SPI1 mode-0 control protocol, the 84-channel ultrasound output engine, the parallel four-WS2812 output, microphone clocking, and the microphone SPI return stream. All four WS2812 devices share one DI line, so the stream intentionally sends one 24-bit GRB value and all four devices receive the same colour.
