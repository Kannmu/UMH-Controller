...
* **AM+PM 音频序列**：以 40 kHz 同步消费左右声道相位控制和独立 8 位包络；包络在每个载波周期内直接控制相位对齐的导通长度。
...
`0x24` 数据帧使用立体声装箱格式，载荷长度为 244 字节：前 4 字节是小端序列号，随后 60 个 `uint32` 样本。每个样本的低 7 位是左相位状态，接着 7 位是右相位状态，再后面各 8 位是左右独立包络。上位机完成累积相位计算，STM32 解包后按每个换能器的相位起点和包络直接构建载波脉宽。
`left:int16, right:int16, envelope:uint8`。上位机以 40 kHz 发送，固件逐样本消费，不再做三次插值。
物理阵列按换能器 X 坐标分成左右两组，两个相位控制和两个包络分别映射到对应分组；包络在 STM32 端以完整 8 位相位对齐脉宽执行。
现有 USB 帧长度上限为 255 字节，因此 50 个记录是单帧可承载的上限。
