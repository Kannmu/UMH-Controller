 ...
 * **单声道聚焦 AM 音频序列**：以 40 kHz 同步消费一个固定相位状态和 8 位包络；序列配置中的点聚焦参数由固件转换为各换能器的校准延时，包络在每个载波周期内控制所有换能器的导通长度。
 * **连续时钟校正**：根据音频环形缓冲相对预缓冲目标的偏差，以最多 ±1000 ppm 的步进微调输入样本消费速度，吸收主机音频时钟与控制器定时器之间的长期误差。
...
* **MCU**: STM32H750VBTx (ARM Cortex-M7, 400MHz)
* **Flash**: 128KB (内部)
* **RAM**: 1MB
...
`0x24` 数据帧使用单声道装箱格式，载荷长度为 244 字节：前 4 字节是小端序列号，随后 120 个 `uint16` 样本。每个样本的低 7 位是固定相位状态，接着 8 位是共享 AM 包络。STM32 根据序列描述符中的点聚焦坐标、每个换能器的校准相位起点和共享包络直接构建载波脉宽。上位机以 40 kHz 发送，固件逐样本消费；单帧覆盖 3 ms 音频，启动预缓冲为 2400 样本，降低 USB 调度开销并增加缓冲余量。
