...
V7 固件在 `DeviceProfile` 中声明 `FOCUSED_AM` 能力位，并提供 `0x90..0x94`
聚焦 AM 音频扩展：主机上传一次固定焦点和 2048 样本包络环的预充深度，之后
只发送 8 位公共 AM 电平。STM32 将相位装载到 FPGA 后，每条音频样本通过
16 字节紧凑 SPI 事务更新 FPGA 非活动事件表，由 FPGA 在载波周期边界交换。
这样既保留 84 路相位校准和通道使能，又把音频流从 216 字节完整帧降到
3 字节热路径（16 字节命令用于旧位流回退），消除了高采样率下的 USB/SPI
卡顿风险。UMH Speaker 上位机自动探测 V7 固件，并把焦点固定在 1000 mm。固件还提供 `FOCUSED_AM_MULTI` 扩展：基础 20 字节配置后可选最多 7 个额外
焦点，所有焦点共享同一条 8 位公共包络。`audio_engine` 只用空间渲染器
合成一次固定相位图，因此多焦点消泡/声束成形不会增加实时 USB 带宽。

同版本新增 `MOTION` 能力位和 `0x62..0x67` 实时焦点运动扩展。主机上传
最多 160 个紧凑路径点（8 字节/点），渲染任务按 50..2000 Hz 直接合成
单个空间点并提交 FPGA；支持单焦点、轴向/横向双焦点、环阵、涡旋和双环
声场，内置临界阻尼跟踪器和最大速度/加速度限幅。手动模式只需 20..50 Hz
的目标点流，设备端即可生成平滑连续运动，用于声悬浮防掉落和视觉暂留
三维光绘。运动数据只存在 STM32 RAM，不写外置 Flash。


板载 GUI 使用 SSD1315 的固定 ASCII 字体，提供首页、播放、设备、校准、存储、Debug、系统、Demo 和控制页面。系统页显示堆、任务数、运行时间和任务 CPU 占用；控制页可执行通用的计划启动、输出停止、计划清空和触发等待操作。Demo 页可选择 ULM、LML 或 LMC，并以 `(0,0,0.1m)` 为中心焦点循环播放；它们复用标准空间渲染、校准、帧环和 FPGA 播放链路。KEY0 至 KEY3 依次为 Return、Confirm、Down、UP。Debug 页按 KEY1 轮换故障摘要、错误计数、FPGA 链路、运行时和设备识别视图。故障摘要保留最后一次故障的短原因、参数、发生时刻和严重等级；严重故障另行保留，不会被后续普通告警覆盖。这样在没有主机连接时，也能仅通过四个按键查看 FPGA 协议、FIFO、播放帧、USB 丢包和存储错误。
