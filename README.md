 ...
 * **单声道 AM+PM 音频序列**：以 40 kHz 同步消费一个相位状态和 8 位包络；包络在每个载波周期内直接控制所有换能器的相位对齐导通长度。
...
`0x24` 数据帧使用单声道装箱格式，载荷长度为 244 字节：前 4 字节是小端序列号，随后 120 个 `uint16` 样本。每个样本的低 7 位是相位状态，接着 8 位是共享包络。上位机完成累积相位计算，STM32 解包后按每个换能器的校准相位起点和共享包络直接构建载波脉宽。上位机以 40 kHz 发送，固件逐样本消费；单帧覆盖 3 ms 音频，启动预缓冲为 2400 样本，降低 USB 调度开销并增加缓冲余量。
